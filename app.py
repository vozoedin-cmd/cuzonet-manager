...
def load_user_from_request(request):
    ...
    if auth_header and auth_header.startswith('Bearer '):
        ...
        if api_token:
            user = Usuario.query.get(api_token.user_id)
            if user:
                return user
    return None
...
class ApiToken(db.Model):
    """Modelo de Tokens API para conexiones de n8n o terceros"""
    __tablename__ = 'api_tokens'
    
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('usuarios.id'), nullable=False)
    token = db.Column(db.String(64), unique=True, nullable=False)
    nombre = db.Column(db.String(100), default='API Key')
    fecha_creacion = db.Column(db.DateTime, default=datetime.utcnow)
    
    usuario = db.relationship('Usuario', backref='api_tokens', lazy=True)
...
def _normalizar_telefono_gt(telefono):
    """Deja solo digitos y antepone 502 a numeros locales de 8 digitos."""
...
def enviar_lote_whatsapp():
    ...
    if not vendedor:
        return jsonify({'success': False, 'error': 'Vendedor no encontrado.'})

    telefono = _normalizar_telefono_gt(vendedor.telefono)
    if not telefono:
    ...
    query = OmadaVoucher.query.filter_by(
        ...
        duracion_unidad=d_un,
        precio=precio
    ).filter(OmadaVoucher.estado == 'activo')
    if lote:
        query = query.filter_by(lote=lote)
    else:
        query = query.filter(OmadaVoucher.lote.is_(None))
